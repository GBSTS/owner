    ...
    <el-container>
          ...
          <div>
            ...
            >
              ...
              <p class="first-p">
                ...
                <!-- 时间 时间格式 -->
                <span class="lastMsgTime" v-if="talk.lastMsgTime != ''" >{{
      ...
      <transition name="el-fade-in-linear">
        ...
        <el-container class="transition-box" v-if="active != ''">
          <el-header>
            <!-- 头部信息 -->
            <span class="teacherName">管理员</span>
          </el-header>
...
export default {
  ...
  methods: {
    /** 刷新列表 */
    getRecordList() {
      talkApi.getRecordList(true,this.id).then((res) => {
        if(res.data.talkRecord[0] == null) {
            this.$data.talk.lastMsgTime = ''
            this.$data.talk.newMsgCount = 0
            this.$data.talk.lastMsgContent = ' '
            this.$data.talk.teacherNumber = this.id
            console.log(this.$data.talk)
        }
        else {
            this.$data.talk = res.data.talkRecord[0]
            console.log(this.$data.talk)
        }
      });
      // console.log("刷新聊天列表 => =>");
    ...
    selectedChat(talk) {
      ...
      // 发送请求，查询聊天记录
      talkApi.getChat(this.id).then((res) => {
    ...
    sendMsg: function (msg) {
      // 请求后端发送信息保存数据库
      this.$data.talkChat.teacherNumber = this.id;
