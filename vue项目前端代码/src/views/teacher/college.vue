<template>
  <div class="app-container">
    <h2 style="color: darkslateblue">学院列表</h2>
    <!-- 查询表单，表单用v-model双向绑定-->
    <el-form :inline="true" class="demo-form-inline">
      ...
      <el-form-item>
        <el-input v-model="dataQuery.name" placeholder="学院名" />
      </el-form-item>
      ...
      <el-button style="margin-bottom: 20px" type="success" icon="el-icon-circle-plus-outline" @click="editForm(id)"
        >添加</el-button
      >
    ...
    <!-- 表单对话框 -->
    <el-dialog title="学院编辑" :visible.sync="dialogEditFormVisible">
      <el-card class="box-card" style="width: 100%">
        ...
        >        
          <div class="el-divider el-divider--horizontal">
            <div class="el-divider__text is-center">学院简介</div>
          </div>
          <div class="el-row">
            <div class="el-col">
              <el-input
                ...
                :rows="3"
                placeholder="请输入内容"
                v-model="college.introduction">
              </el-input>
            </div>
          </div>
            <div class="el-divider el-divider--horizontal">
              <div class="el-divider__text is-center">学院信息</div>
            </div>
          <div class="el-row">
            <div class="el-col el-col-12">
              <el-form-item label="学院名称" prop="name">
                <el-input v-model="college.name" style="width: 80%"></el-input>
              </el-form-item>
            </div>
            <div class="el-col el-col-10">
              <el-form-item label="所属学校" prop="schoolId">
                <el-select style="width: " v-model="college.schoolId" placeholder="请选择学院">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="saveOrUpdate('college')">确 定</el-button>
      </div>
    </el-dialog>

    <!-- 详情对话框 -->
    <el-dialog title="学院详情" :visible.sync="dialogInfoFormVisible">
      <el-card class="box-card" style="width: 100%" :body-style="{ padding: '0px' }">
      <h2 style="text-align: center">{{ college.name }}</h2>
        ...
        <div class="el-row">
          <div class="el-col el-col-12">
            <div class="el-form-item">
              <label class="el-form-item__label">学院名称:</label>
              <div class="el-form-item__content">
                <el-label>{{ college.name }}</el-label>
              </div>
            </div>
          </div>
          <div class="el-col el-col-12">
            <div class="el-form-item">
              <label class="el-form-item__label">所属学校:</label>
              <div class="el-form-item__content">
                <el-label>{{ college.schoolName }}</el-label>
              </div>
            </div>
          </div>
        </div>
        <div class="el-row">
          <div class="el-col el-col-12">
            <div class="el-form-item">
              ...
              <div class="el-form-item__content">
                <el-label>{{ college.gmtCreate }}</el-label>
              </div>
            </div>
          </div>
          <div class="el-col el-col-12">
            <div class="el-form-item">
              ...
              <div class="el-form-item__content">
                <el-label>{{ college.gmtModified }}</el-label>
              </div>
            </div>
          </div>
        </div>
...
import schoolApi from '@/api/te/school'
  ...
  methods: {
    ...
    handleDownload() {
      ...
      import("@/vendor/Export2Excel").then((excel) => {
        const tHeader = ["学院名", "所属学校", "创建时间", "更新时间"]; //表头名称
        const filterVal = ["name", "schoolName", "gmtCreate", "gmtModified"];
    ...
    picturePreview(name) {
      if (name == "photo") {
        this.dialogImageUrl = this.college.photo;
      } else if (name == "lecense") {
        this.dialogImageUrl = this.college.lecense;
      }
    ...
    handleAvatarSuccess(res, file) {
      this.college.photo = URL.createObjectURL(file.raw);
    },
    //上传头像之前
    ...
    infoClicked(id) {
      ...
      deleteSeleted() {
        ...
        if(role == 'admin' || role == 'admin_teacher' || role == 'school') {
          this.$confirm("此操作将永久删除记录, 是否继续?", "提示", {
            ...
            collegeApi.delCollegesByList(idList)
              .then(res=>{
                this.$message({
                  type: "success",
                  message: "删除成功!",
    ...
    //获取所有数据
    getList() {
      collegeApi.pageList(this.page,this.limit,this.dataQuery)
        .then(res=>{
    ...
    saveOrUpdate(college) {
      ...
      this.$refs[college].validate((valid) => {
        if (valid) {
          //表单前端校验通过
          if(this.college.id == null || this.college.id == "") {
            for(var i in  _this.list)
            {
              if(_this.list[i].name==this.college.name)
              {
                is=1
              }
              if(this.college.gmtCreate>this.college.gmtModified)
              {
                _is=1
              }
            }
        ...
        if(is)
          ...
          type: 'warning'
            ...
            if(!is&&!_is)
            {
            collegeApi.saveCollege(this.college)
        ...
        if(_is)
        {
          ...
          type: 'warning'
            ...
            if(!_is)
            {
            collegeApi.updateCollege(this.college)
            .then(res=>{
              this.$message({
                type: "success",
                message: "编辑成功!",
